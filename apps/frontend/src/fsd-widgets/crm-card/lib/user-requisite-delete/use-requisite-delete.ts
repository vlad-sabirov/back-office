...
export const useRequisiteDelete = () => {
	const current = useStateSelector((state) => state.crm_organization.data.current);
	const [updateRequisiteFetch] = CrmOrganizationRequisiteService.deleteById();
	...
	return useCallback(
		async (requisite: ICrmOrganizationRequisiteFormEntity): Promise<boolean> => {
			...
			const response = await updateRequisiteFetch(requisite.id ?? 0);
			...
			const message =
				`${user.sex === 'male' ? 'Удалил' : 'Удалила'} реквизиты` +
