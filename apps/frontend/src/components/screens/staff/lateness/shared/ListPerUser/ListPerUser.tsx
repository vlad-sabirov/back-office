...
import { ListPerUserProps } from '.';
import css from './list-per-user.module.scss';

export const ListPerUser: FC<ListPerUserProps> = ({ data, onSuccess }) => {
	...
	useEffect(() => {
		if (!latenessData || !latenessData.length) return;
		...
		setTableData((old) => ({
			...
			body: dataRange.map((dateItem) => {
				...
				const isHoliday = holidays.some((itemHoliday) => itemHoliday.includes(format(dateItem, 'yyyy-MM-dd')));
				...
				const isDisplayAction = CheckAccess(Access.cabinetDisplayAttendanceAction);
				...
				const TimeDisplay = isAfterRegister ? (
					...
					<TextField
						className={cn(css.time, {
							[css.time__arrived]: timeToNumber && timeToNumber / 1000 <= 905,
							[css.time__late]: timeToNumber && timeToNumber / 1000 > 905,
						})}
						disabled={isDisabled}
					>
						{findLateness.time}
				...
				const CommentDisplay = (
					<>
						{findLateness?.isSkipped && (
							<TextField
								className={cn(css.time, css.time__isSkipped)}
								size={'small'}
								disabled={isDisabled}
							>
								Не учитывается
								...
								{findLateness.comments.map((commentItem) => {
									...
									return (
										<Menu
											offset={-20}
											key={commentItem.id}
											disabled={!isDisplayAction}
											control={
												<div
													className={css.comments__item}
													style={{
														pointerEvents: !isDisplayAction ? 'none' : undefined,
														userSelect: !isDisplayAction ? 'none' : undefined,
													}}
												>
													{!!commentItem.user && (
				...
				const ActionDisplay = isDisplayAction ? (
					<Menu
						disabled={isAfterDay || isHoliday}
						control={
							<div className={css.actionMenu}>
								{!isAfterDay && !isHoliday && <Icon name={'dots-three'} />}
							</div>
						}
					>
						<Menu.Item
