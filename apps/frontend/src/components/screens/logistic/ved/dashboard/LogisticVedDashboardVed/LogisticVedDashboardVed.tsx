...
import { LogisticVedDashboardVedProps } from './props';
import css from './styles.module.scss';

export const LogisticVedDashboardVed: FC<LogisticVedDashboardVedProps> = ({ className, ...props }) => {
	...
	const handleRefresh = async () => {
		const [findOrders] = await LogisticService.findActiveWithRole('logisticVedOrdersVed');
			...
			<ContentBlock className={css.root}>
				{isLoading ? (
					...
					<>
						...
						<Tooltip
							label={'Список всех заявок ВЭД, требующего внимания твоего отдела'}
							withArrow
							openDelay={1000}
							transitionDuration={300}
							position={'top-start'}
						>
							<div>
								<TextField className={css.orderCount}>
									Заявки требующие внимания: <span>{orders.length}</span>
								</TextField>
							</div>
						</Tooltip>
