...
import { useRandom, useUserDeprecated } from '@hooks';
...
import { LogisticVedDashboardAuthorProps } from './props';
import css from './styles.module.scss';

export const LogisticVedDashboardAuthor: FC<LogisticVedDashboardAuthorProps> = ({ className, ...props }) => {
	...
	const handleRefresh = async () => {
		if (!team) return;
		const [findOrders] = await LogisticService.findActive(team);
		if (!findOrders) return;

		setOrderModificationCount(findOrders.filter((order) => order.isModification).length);
		setOrderYellowCount(
			findOrders
				.filter((order) => order.isModification || rolesAlias?.includes(order.stage.actionExpected.alias))
				.filter((order) => {
			...
			<ContentBlock className={css.root}>
				{isLoading ? (
					...
					<>
						...
						<Tooltip
							label={`Список ${
								team && team.length > 1 ? ' заявок ВЭД твоей команды' : 'твоих заявок ВЭД'
                    ...
                    Время потраченное на их исполнение сильно превышает запланированное.`}
							...
							<Tooltip
								label={`Неправильно оформленные заявки, отправленные ${
									team && team.length ? 'твоей команде' : 'тебе'
								} на доработку`}
								withArrow
								width={320}
								multiline
