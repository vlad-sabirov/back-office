...
export const Team: FC<ITeamProps> = (props) => {
	...
	const plan = useMemo(() => {
		return values.teams?.[user.id]?.plan ?? 0;
	}, [user.id, values.teams]);

	const planCustomerCount = useMemo(() => {
		return values.teams?.[user.id]?.planCustomerCount ?? 0;
	}, [user.id, values.teams]);

	const planWorkingBasePercent = useMemo(() => {
		return values.teams?.[user.id]?.planWorkingBasePercent ?? 0;
	}, [user.id, values.teams]);
	...
	return (
		<>
			...
			{/* {user.child && user.child.length > 1 && (
				<>
					<Employee
						user={user}
